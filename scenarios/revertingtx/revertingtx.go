...
import (
	"context"
	"fmt"
	revertingtx "github.com/astriaorg/spamooor/scenarios/revertingtx/contracts"
...
func NewScenario() scenariotypes.Scenario {
	return &Scenario{
		logger: logrus.WithField("scenario", "revertingtx"),
	}
}
...
func (s *Scenario) Setup(testerCfg *tester.Tester) error {
	s.tester = testerCfg
	s.logger.Infof("setting up scenario: revetingtx")
	s.logger.Infof("deploying reverting tx contract...")
	receipt, _, err := s.DeployRevertingTxContract()
	if err != nil {
		return err
	}

	s.revertingContractAddr = receipt.ContractAddress

	s.logger.Infof("deployed reverting tx contract at %v", s.revertingContractAddr.String())
...
func (s *Scenario) Run() error {
	...
	s.logger.Infof("starting scenario: revertingtx")
...
func (s *Scenario) DeployRevertingTxContract() (*types.Receipt, *txbuilder.Client, error) {
	...
	_, deployTx, _, err := revertingtx.DeployRevertingtx(transactor, client.GetEthClient())
	if err != nil {
		return nil, nil, err
	}
...
func (s *Scenario) sendTx(txIdx uint64) (*types.Transaction, *txbuilder.Client, error) {
	...
	transferFnSignature := []byte("alwaysRevert()")
...
func (s *Scenario) GetRevertingContract() (*revertingtx.Revertingtx, error) {
	client := s.tester.GetClient(tester.SelectByIndex, 0)
	return revertingtx.NewRevertingtx(s.revertingContractAddr, client.GetEthClient())
}
